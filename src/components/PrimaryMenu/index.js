  ...
  return (
    ...
    >
      <div className="menuItem__wrap">
        ...
        <span className="menuItem__toggle"
  ...
  <StaticQuery
    ...
    render={ data => {
      ...
      let structuredMenu = topLevel.map( item => {
        ...
        const children = menuItems.filter( subItem => subItem.parentId === item.id )
      ...
      return (
        <nav className={`menuWrapper`}>
          ...
          {
            structuredMenu.map(item => (item.children && item.children.length) ? <ItemSubMenu key={item.id} item={item} items={item.children}/> : <Item key={item.id} item={item}/>)
          }
          </ul>
        </nav>
      )
