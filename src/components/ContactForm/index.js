  ...
  mutation CreateSubmissionMutation(
    ...
    submitGravityFormsForm(
      input: {
        ...
        fieldValues: [
          { id: 8, value: $name }
          { id: 4, emailValues: { value: $email } }
          { id: 5, value: $phone }
          { id: 6, checkboxValues: $interest }
          { id: 7, value: $message }
        ]
      }
    ) {
  ...
  return (
    <Mutation mutation={CONTACT_MUTATION}>
      {(submitGravityFormsForm, { loading, error, data }) => {
        ...
        if (loading) {
          return (
            <div className="formResponse formResponse--loading">
              Submiting form...
            </div>
          )
        }
        ...
        if (data && !data?.submitGravityFormsForm?.errors) {
          return (
            <div className="formResponse formResponse--thankYou">
              Thank you!
            </div>
          )
        }
          ...
          <form onSubmit={e => handleSubmit(e, submitGravityFormsForm)}>
            {fields &&
              fields.map(field => {
                ...
                const fieldKey = String(field.adminLabel)
                  .toLowerCase()
                  .replace(/\s/g, "")
