...
const Portfolio = ({ data }) => {
  const { title, excerpt, featuredImages } = data?.wpPortfolio || [
    { title: null, excerpt: null, featuredImages: { images: [] } },
  ]

  // Set portfolioImages.
  const portfolioImages = useMemo(() => featuredImages?.images || [], [
    featuredImages,
  ])
  ...
  const featuredPortfolioImage =
    portfolioImages && portfolioImages.length ? portfolioImages[0] : []
  ...
  useEffect(() => {
    const featuredImage = portfolioImages.filter(
      (image, index) => index === currentIndex
    )
    if (featuredImage && featuredImage.length)
      setFeaturedImage(featuredImage[0])
    ...
    <Layout>
      ...
      <div className="portfolio">
        <div className="portfolio__featured" {...handlers}>
          {featuredImage && (
            <GatsbyImage
              image={getImage(featuredImage.featured)}
